...
@synthesize startNewSearchBtn,continueSearchBtn,viewSearchResultBtn;
...
- (void)viewDidLoad
{
    ...
    [infoDarkButtonType addTarget:self action:@selector(navPage) forControlEvents:UIControlEventTouchUpInside];
...
-(IBAction)continueSearch:(id)sender{
    UIAlertView *alert= [[UIAlertView alloc]initWithTitle:@"Message" message:@"Please Upgrade to Pro Version" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:nil, nil];
    [alert show];
    [alert release];    
...
-(void)navPage{
    ...
    InfoButton *infoObj = [[InfoButton alloc]init];
...
- (void)dealloc
{
    [startNewSearchBtn release];
    [viewSearchResultBtn release];
    [continueSearchBtn release];
    [super dealloc];
}
